...
| Absolute Magnitude                            | Luminosity of a celestial object, on an _inverse logarithmic astronomical magnitude scale_. <br> <br> The absolute magnitude of a star is equal to the apparent magnitude that the object would have if it were viewed from a distance of exactly 10 parsecs (32.6 light-years), without extinction of its light due to absorption by interstellar matter and cosmic dust. By placing all objects at a standard distance from the observer, _their luminosities are comparable on a magnitude scale_. <br> [absolute-magnitude-wiki](https://en.wikipedia.org/wiki/Absolute_magnitude)                                                            |
...
| :-------------------- | :-------------------- | :---------------------- |
| 0                     | Red Dwarf             | Smallest, longest burning (life), lowest temperature star. <br> "A red dwarf, which is half as massive as the Sun, can last 80 to 100 billion years, which is far longer than the universe's age of 13.8 billion years. This long life is one reason red dwarfs are considered good sources for planets hosting life, because they are stable for such a long time." <br>[red-dwarf](https://www.space.com/23772-red-dwarf-stars.html)                       | 
...
- Dropped the `luminosity` column from the dataset and kept `absolute_magnitude`. Absolute magnitude measures a star's luminosity, regardless of distance in space.
...
- Created 3 pair plots to visualize the interactions between continuous features.
...
__Is the spectral class dependent on color?__
> 𝐻0: `Star type` is independent of `color`.
> 
> 𝐻1: `Star type` is dependent on `color`.
>
...
### 5. Conclusion
__New stars we observe are easily classified using `absolute_magnitude` and `radius`.__

### 6. Future Investigations
Given the small size of the dataset, I wonder if new stars would impact the model's prediction capability.
